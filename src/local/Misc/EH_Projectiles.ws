...
class W3ACSKnifeProjectile extends W3AdvancedProjectile
{
	...
	function DealDamageToTarget( victim : CGameplayEntity, eff : EEffectType, crit : bool )
	{
		...
		&& actortarget.IsAlive() ) 
		{
			...
			action.Initialize(GetWitcherPlayer(),actortarget,this,GetWitcherPlayer(),EHRT_Heavy,CPS_Undefined,false,true,false,false);
			...
			action.AddEffectInfo( EET_Bleeding, 10 );
...
statemachine class CACSEverstormLightning extends CGameplayEntity
{
	...
	function deal_damage( victimtarget : CActor )
	{
		...
		if ( victimtarget && victimtarget.IsAlive() ) 
		{
			...
			action = new W3DamageAction in theGame.damageMgr;
			action.Initialize((CGameplayEntity)this,victimtarget,this,this.GetName(),EHRT_None,CPS_Undefined,false,false,true,false);
			action.SetProcessBuffsIfNoDamage(true);
			action.SetCanPlayHitParticle( true );
	...
	timer function Explode( deltaTime : float, optional id : int )
	{
		...
		for( i = 0; i < actors.Size(); i += 1 )
		{
			...
			{ 
				...
				|| ((CActor) actors[i]).HasAbility('Boss')
				|| ((CActor) actors[i]).HasTag('IsBoss')
				)
				{
...
class W3ACSClawSpikesProjectile extends W3TraceGroundProjectile
{
	private var action 						: W3DamageAction;
	private var damage						: float;
	...
	event OnProjectileCollision( pos, normal : Vector, collidingComponent : CComponent, hitCollisionsGroups : array< name >, actorIndex : int, shapeIndex : int )
	{
		...
		if ( victim && !collidedEntities.Contains(victim) )
		{
			...
			if (((CActor)victim).UsesEssence())
			{
				damage = ((CActor)victim).GetStat( BCS_Essence ) * 0.15;
			}
			else if (((CActor)victim).UsesVitality())
			{
				damage = ((CActor)victim).GetStat( BCS_Vitality ) * 0.15;
			}
...
class W3ACSDullahanProjectile extends W3AdvancedProjectile
{
	editable var initFxName 			: name;
	editable var onCollisionFxName 		: name;
	editable var spawnEntityTemplate 	: CEntityTemplate;
	...
	private var customHorseEnt : W3ACSCustomWingEntity;
	...
	event OnProjectileInit()
	{
		...
		AddTimer('DullahanProjCheckTag', 0.1, false);
	}

	timer function DullahanProjCheckTag( time : float , id : int )
	{
		if (this.HasTag('ACS_Dullahan_Spawning_Proj'))
		{
			PlayEffect('explosion');
		}
		else if (this.HasTag('ACS_Dullahan_Steed_Spawning_Proj'))
		{
			PlayEffect('explosion');
		}
		else if (this.HasTag('ACS_Dullahan_Vertical_Proj'))
		{
			AddTimer('ACS_Dullahan_Vertical_Proj_Init', 3, false);
		}
		else if (this.HasTag('ACS_Dullahan_Glow_Proj'))
		{
			AddTimer('ACS_Dullahan_Glow_Proj_Init', 3, false);
		}
		else if (this.HasTag('ACS_Dullahan_Small_Proj'))
		{
			AddTimer('ACS_Dullahan_Small_Proj_Init', 3, false);
		}
		else if (this.HasTag('ACS_Dullahan_Horse_Riding_Proj'))
		{
			AddTimer('ACS_Dullahan_Horse_Riding_Proj_Init', 3, false);
		}
		else if (this.HasTag('ACS_Dullahan_Spear_Player_Horse_Riding_Proj'))
		{
			AddTimer('ACS_Dullahan_Spear_Player_Horse_Riding_Proj_Init', 3, false);
		}
	}

	timer function ACS_Dullahan_Vertical_Proj_Init( time : float , id : int )
	{
		CreateDullahanHorse();
		AddTimer('DullahanProjHoming', 0.00001, true);
	}

	timer function ACS_Dullahan_Glow_Proj_Init( time : float , id : int )
	{
		CreateDullahanGlowHorse();
		AddTimer('DullahanProjHoming', 0.00001, true);
	}

	timer function ACS_Dullahan_Small_Proj_Init( time : float , id : int )
	{
		...
		{
			PlayEffect('explosion');
			StopEffect('explosion');

			AddTimer('DullahanSmallProjHoming', 0.00001, true);
		}
	}

	timer function ACS_Dullahan_Horse_Riding_Proj_Init( time : float , id : int )
	{
		...
		if (distance_from_player <= 15*15)
		{
			PlayEffect('explosion');
			StopEffect('explosion');

			this.ShootProjectileAtPosition( 0, 10, ACSFixZAxis(targetPosition), 500 );
		}
	}

	timer function ACS_Dullahan_Spear_Player_Horse_Riding_Proj_Init( time : float , id : int )
	{
		var targetPosition : Vector;

		if (thePlayer.GetTarget())
		{
			targetPosition = (thePlayer.GetTarget()).GetWorldPosition();
		}
		else
		{
			targetPosition = theCamera.GetCameraPosition() + VecFromHeading(theCamera.GetCameraHeading()) * 30;
		}
	...
	timer function DullahanSmallProjHoming( time : float , id : int )
	{
		var targetPosition : Vector;

		targetPosition = thePlayer.PredictWorldPosition(0.7f);

		this.ShootProjectileAtPosition( 0, 10, targetPosition, 500 );
	}

	var DullahanProjHomingDelta : float;

	timer function DullahanProjHoming( time : float , id : int )
	{
		var targetPosition : Vector;

		targetPosition = thePlayer.PredictWorldPosition(0.7f);

		DullahanProjHomingDelta += 0.1;

		this.ShootProjectileAtPosition( 0, 10 + DullahanProjHomingDelta, targetPosition, 500 );
	}

	function CreateDullahanHorse()
	{
		PlayEffect('explosion');
		StopEffect('explosion');

		customHorseEnt = (W3ACSCustomWingEntity)theGame.CreateEntity( (CEntityTemplate)LoadResource( 

		"dlc\dlc_acs\data\entities\other\custom_wings\custom_dullahan_horse.w2ent"

		, true ), GetWorldPosition(), GetWorldRotation() );
		
		customHorseEnt.CreateAttachment( this, , Vector( 0, -1, 0 ), EulerAngles(0,0,0) );

		customHorseEnt.PlayEffect('fire_fx_default');
	}

	function CreateDullahanGlowHorse()
	{
		PlayEffect('explosion');
		StopEffect('explosion');
		...
		"dlc\dlc_acs\data\entities\other\custom_wings\custom_dullahan_horse_glow.w2ent"
	...
	protected function DealDamageToVictim( victim : CGameplayEntity )
	{
		...
		if ( victim == ACSGetCActor('ACS_Dullahan') )
		{
			projDMG = 0;
		...
		else
		{
			...
			if(ACSGetCActor('ACS_Dullahan').GetCurrentHealth() >= ACSGetCActor('ACS_Dullahan').GetMaxHealth() * 0.5)
			{
				projDMG * 0.75;
			}
	...
	function DullahanProjCleanup()
	{
		var dRotation : EulerAngles;

		customHorseEnt.Destroy();

		RemoveTimer('DullahanProjHoming');

		RemoveTimer('DullahanSmallProjHoming');

		RemoveTimer('DullahanHorseRidingProjHoming');

		if (this.HasTag('ACS_Dullahan_Vertical_Proj'))
		{
			ACSGetCActor('ACS_Dullahan').BreakAttachment();

			((CAnimatedComponent)ACSGetCActor('ACS_Dullahan').GetComponentByClassName('CAnimatedComponent')).UnfreezePoseFadeOut(0.01f);

			CreateSpearSpikes();

			//ACSGetCActor('ACS_Dullahan').AddTimer('ACS_DullahanEnableAttackDelay', (RandRangeF(5,3)), false);

			dRotation = ACSGetCActor('ACS_Dullahan').GetWorldRotation();

			dRotation.Pitch = 0;

			dRotation.Roll = 0;

			ACSGetCActor('ACS_Dullahan').SetVisibility(true);

			ACSGetCActor('ACS_Dullahan').PlayEffectSingle('fire_fx_default');

			ACSGetCActor('ACS_Dullahan').PlayEffectSingle('ghost');

			ACSGetCActor('ACS_Dullahan').TeleportWithRotation(ACSGetCActor('ACS_Dullahan').GetWorldPosition(), dRotation);
		}
		else if (this.HasTag('ACS_Dullahan_Spawning_Proj'))
		{
			Spawn_Dullahan();
			CreateSpearSpikes();
		}
		...
		{
			Spawn_Dullahan_Steed();
			CreateSpearSpikes();
		}
	}

	function Spawn_Dullahan()
	{
		var ent	: CACSMonsterSpawner;
		...
		ent.AddTag('ACS_MonsterSpawner_Dullahan_Actual');
	}

	function Spawn_Dullahan_Steed()
	{
		var temp, temp_2, temp_3											: CEntityTemplate;
		var ent, ent_2, ent_3												: CEntity;
		var i, count														: int;
		var playerPos, spawnPos, newSpawnPos								: Vector;
		var randAngle, randRange											: float;
		var meshcomp, meshcomp2												: CComponent;
		var animcomp, animcomp2												: CAnimatedComponent;
		var h 																: float;
		var bone_vec, pos, attach_vec										: Vector;
		...
		ACSGetCEntity('ACS_Dullahan_Horse_Player_Summon').Destroy();

		ACSGetCEntity('ACS_Dullahan_Steed_Fire_Sword_Ground_Fx_Ent').Destroy();

		temp_2 = (CEntityTemplate)LoadResource( 

		"dlc\dlc_acs\data\entities\enemy_riders\horse_vehicle_dullahan.w2ent"
		...
		playerRot.Yaw += RandRangeF(225, 135);

		adjustedRot = EulerAngles(0,0,0);

		adjustedRot.Yaw = playerRot.Yaw;

		ent_2 = theGame.CreateEntity(temp_2, ACSPlayerFixZAxis(playerPos), playerRot,true,false,false,PM_DontPersist);

		animcomp2 = (CAnimatedComponent)ent_2.GetComponentByClassName('CAnimatedComponent');
		meshcomp2 = ent_2.GetComponentByClassName('CMeshComponent');
		h = 1;
		animcomp2.SetScale(Vector(h,h,h,1));
		meshcomp2.SetScale(Vector(h,h,h,1));	

		ent_2.AddTag( 'ACS_Dullahan_Horse_Player_Summon' );

		ent_2.PlayEffect('fire_fx_default');

		((CActor)ent_2).SetTemporaryAttitudeGroup( 'friendly_to_player', AGP_Default );
		
		((CNewNPC)ent_2).SetAttitude(thePlayer, AIA_Friendly);

		((CActor)ent_2).AddAbility( 'DisableHorsePanic' );

		((CActor)ent_2).SetAnimationSpeedMultiplier(1.25);
		...
		ent_ground_fx.AddTag('ACS_Dullahan_Steed_Fire_Sword_Ground_Fx_Ent');

		ent_ground_fx.CreateAttachment( ent_2, , Vector( 0, 2, 0 ), EulerAngles(0,90,0) );

		ent_ground_fx.PlayEffectSingle('golem_ground_fire_fx_dullahan');
	...
	function CreateSpearSpikes()
	{
		var temp																	: CEntityTemplate;
		var proj_1																	: W3ACSDullahanSpearsProjectile;
		var i, count																: int;
		var initPos, spawnPos														: Vector;
		...
		"dlc\dlc_acs\data\entities\projectiles\dullahan_spears.w2ent"
		...
		for( i = 0; i < count; i += 1 )
		{
			...
			proj_1 = (W3ACSDullahanSpearsProjectile)theGame.CreateEntity( temp, ACSFixZAxis(spawnPos) );
			

			proj_1.Init(NULL);
			proj_1.PlayEffectSingle('fire_line');
			proj_1.ShootProjectileAtPosition(0,	15, ACSFixZAxis(initPos), 4 );
			proj_1.DestroyAfter(10);
...
class W3ACSDullahanSpearsProjectile extends W3TraceGroundProjectile
{
	private var action 						: W3DamageAction;
	private var damage						: float;
	private var customHorseEnt 				: W3ACSCustomWingEntity;
	...
	event OnProjectileInit()
	{
		...
		AddTimer('DullahanGroundProjCheckTag', 0.1, false);
	}

	timer function DullahanGroundProjCheckTag( time : float , id : int )
	{
		if (this.HasTag('ACS_Dullahan_Homing_Spear_Proj'))
		{
			CreateDullahanGlowHorse();

			AddTimer('ACS_Dullahan_Homing_Spear_Proj_Init', 3, false);

			AddTimer('ACS_Dullahan_Homing_Spear_Proj_DestroyAfter', 10, false);
		}
	}

	timer function ACS_Dullahan_Homing_Spear_Proj_Init( time : float , id : int )
	{
		AddTimer('DullahanGroundProjHoming', 0.00001, true);
	}

	timer function ACS_Dullahan_Homing_Spear_Proj_DestroyAfter( time : float , id : int )
	{
		RemoveTimer('DullahanGroundProjHoming');
	}
	...
	function CreateDullahanGlowHorse()
	{
		...
		customHorseEnt.CreateAttachment( this, , Vector( 0, 0.5, 0 ), EulerAngles(0,0,0) );

		customHorseEnt.PlayEffect('fire_fx_default');
	}

	timer function DullahanGroundProjHoming( time : float , id : int )
	{
		var distance_from_player: float;
		var targetPosition : Vector;
		...
		if (distance_from_player <= 1.5*1.5)
		{
			RemoveTimer('DullahanGroundProjHoming');
		...
		this.ShootProjectileAtPosition( 0, 10, ACSFixZAxis(targetPosition), 10 );
	}
	...
	event OnProjectileCollision( pos, normal : Vector, collidingComponent : CComponent, hitCollisionsGroups : array< name >, actorIndex : int, shapeIndex : int )
	{
		...
		)
		{
			action = new W3DamageAction in this;
			action.Initialize((CGameplayEntity)caster,victim,this,caster.GetName(),EHRT_Heavy,CPS_Undefined,false,true,false,false);
			action.AddEffectInfo(EET_Bleeding, 2.0);

			if (((CActor)victim).UsesEssence())
			{
				damage = ((CActor)victim).GetStatMax( BCS_Essence ) * 0.3;
			}
			else if (((CActor)victim).UsesVitality())
			{
				damage = ((CActor)victim).GetStatMax( BCS_Vitality ) * 0.3;
			}
...
class W3ACSAssassinBolt extends W3BoltProjectile
{
	private var boltVictim 															: CActor;
	...
	event OnProjectileCollision( pos, normal : Vector, collidingComponent : CComponent, hitCollisionsGroups : array< name >, actorIndex : int, shapeIndex : int )
	{
		...
		{
			dist = (((CMovingPhysicalAgentComponent)((CActor)victim).GetMovingAgentComponent()).GetCapsuleRadius()
			...
			movementAdjustorFinisher.AdjustmentDuration( ticketFinisher, 0.125 );
			...
			movementAdjustorFinisher.SlideTowards( ticketFinisher, victim, dist, dist );
			movementAdjustorFinisher.RotateTowards( ticketFinisher, victim );
			...
			GetACSWatcher().CrossbowHide(true);
			...
			thePlayer.BlockAllActions( 'ACS_Finisher_Bolt_Action_Block', true );

			AddTimer( 'assassin_bolt_finisher_check', 0.000001, true );
		}
	}

	timer function assassin_bolt_finisher_check( time : float , id : int )
	{
		var targetDistance 										: float;

		targetDistance = VecDistanceSquared2D( thePlayer.GetWorldPosition(), boltVictim.GetWorldPosition() ) ;

		if(thePlayer.IsInCombat() && targetDistance <= 2 * 2)
		{
			AddTimer( 'assassin_bolt_finisher', 0.0125, false );

			RemoveTimer('assassin_bolt_finisher_check');
		}
	}

	timer function assassin_bolt_finisher( time : float , id : int )
	{
		var actor : CActor;

		RemoveTimer('assassin_bolt_finisher_check');

		actor = (CActor)(boltVictim);

		thePlayer.SetVisibility(true);

		GetACSWatcher().CrossbowHide(false);
